describe('Cenários de Teste: GET /carrinhos/{_id}', () => {
  ...
  before(() => {
    ...
    // 1. Criar e logar com Admin para prover Produto e Carrinho
    cy.request('POST', '/usuarios', {
      ...
      cy.request('POST', '/login', { email: emailAdmin, password: "teste" }).then(res => {
        ...
        // 2. Criar um produto para colocar no carrinho
        cy.request({
          method: 'POST',
          url: '/produtos',
          ...
          // 3. Criar o carrinho e capturar o ID dele
          cy.request({
            method: 'POST',
            url: '/carrinhos',
            headers: { authorization: token },
  ...
  it('Status 400: Deve retornar erro ao buscar carrinho inexistente.', () => {
    ...
    cy.request({
      ...
      // Validação baseada no screenshot do erro
