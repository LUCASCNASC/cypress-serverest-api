describe('Endpoint - DELETE /carrinhos/cancelar-compra', () => {
  let tokenValido;
  ...
  beforeEach(() => {
    ...
    // Criar e logar com Admin para prover produto com estoque
    cy.request('POST', '/usuarios', {
      ...
      cy.request('POST', '/login', { email: emailAdmin, password: "teste" }).then(res => {
        ...
        cy.request({
          ...
          // Criar usuário comprador e logar
          cy.request('POST', '/usuarios', {
  ...
  it('Status code as 200: It should complete the purchase successfully.', () => {
    cy.request({
      method: 'DELETE',
  ...
  it('Status code as 401: It should return an error when trying to complete without an access token.', () => {
    cy.request({
      method: 'DELETE',
      url: '/carrinhos/concluir-compra',
